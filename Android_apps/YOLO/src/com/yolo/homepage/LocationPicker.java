...
public class LocationPicker extends Activity implements LocationListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		returnIntent.putExtra("company", intent.getStringExtra("company"));
		returnIntent
		...
		locationlist.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view,
				...
				if (itemPosition == 0) {
					...
					boolean connected = NetworkUtil.isNetworkAvailable(getApplicationContext());
					if (!connected) {
						alertConnection();
					} else {
						getUserAddress();
			...
			*/		ssds.setCity(itemValue);
					ssds.setState("CA");
					/*SearchScreen screen = new SearchScreen();
					screen.locationView = (TextView)findViewById(R.id.location);
					screen.locationView.setText(itemValue);
					finish();*/
	...
	public void onLocationChanged(Location location) {
		...
		try {
			...
			{
				if(city.toLowerCase().contains("san francisco"))
				{	
					ssds.setCity(city);
					ssds.setState(state);
				}
				...
				{
					Toast.makeText(getApplicationContext(), "Will be available in "+city+" soon.", Toast.LENGTH_SHORT).show();
				}
