...
public class SplitImage extends Activity implements OnClickListener{
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if(first_time)
		{
			original_imageChunks= getIntent().getParcelableArrayListExtra("original_image_chunks");
			image_id = new int[imageChunks.size()];
			for(int i =0;i<imageChunks.size()-1;i++)
			{
				image_id[i] = imageChunks.size()-1-i;
			}
		...
		moves_count = getIntent().getIntExtra("moves_count", 0);
		leftTime = getIntent().getLongExtra("remaining_time", startTime);
		...
		no_cols=no_rows=(int) Math.sqrt(imageChunks.size());
	...
	private void displayDialog() {
		...
		AlertDialog dialog = new AlertDialog.Builder(SplitImage.this).create();
		...
		dialog.setButton(DialogInterface.BUTTON_NEGATIVE, "Restart Game",
				new DialogInterface.OnClickListener() {
					...
					public void onClick(DialogInterface dialog, int which) {
						...
						game = new Intent(getApplicationContext(), MainActivity.class);
						game.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP);
	...
	public void onClick(View v) {
		//Toast.makeText(getApplicationContext(), "Image view clicked with id: "+v.getId(), Toast.LENGTH_SHORT).show();
	...
	private void checkAndChange(String number) {
		...
		if (swap_possible) {
           ...
           // imageChunks.set(imageChunks.size()-1, null);
    		...
    		intent.putExtra("remaining_time", remainingTime-1000);
